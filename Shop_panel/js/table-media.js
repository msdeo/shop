...
// 	// append blank row at the end of table2
// 	$("."+table2).append(blankrow);
...
// 			$("."+table2+" tr:last-child").fadeOut("fast");
...
// 	$(document).on("dblclick","."+table2+" td",function(e){
...
// 	});
	...
	$(document).on("blur","."+table2+" td",function(e){
		if(tdediting == 1){
			var newval = $("."+table2+" tr[id='"+editingtrid+"'] td[class='"+editingtdcol+"']").find(inputs).val();
...
ajax = function (params,action){
	$.ajax({
		type: "POST", 
		url: "test2.php?"+params+"&action="+action,
		success: function(response){
		  ...
		  switch(action){
				...
				var seclastRow = $("."+table2+" tr").length;
...
// 											html +='<td class="address">'+myObj.address+'</td>';
					...
					// Append new row as a second last row of a table2
					$("."+table2+" tr").last().before('<tr id="'+myObj.id+'">'+html+'</tr>');
					
					if(effect == "slide"){
						// Little hack to animate TR element smoothly, wrap it in div and replace then again replace with td and tr's ;)
						$("."+table2+" tr:nth-child("+seclastRow+")").find('td')
					   ...
					   $("."+table2+" tr:nth-child("+seclastRow+")").effect("highlight",{color: '#acfdaa'},100);
					}else
					   $("."+table2+" tr:nth-child("+seclastRow+")").effect("highlight",{color: '#acfdaa'},1000);
...
// 				}
			break;
			case "del":
				 seclastRow = $("."+table2+" tr").length;
// 				if(response.success == 1){
					$("."+table2+" tr[id='"+myObj.id+"']").effect("highlight",{color: '#f4667b'},500,function(){
						$("."+table2+" tr[id='"+myObj.id+"']").remove();
					});
// 				}
			break;
			case "update":
...
// 	$("tr[id='"+myObj.id+"'] td[class='"+columns[4]+"']").html(myObj.address);
			...
			case "updatetd":
				...
				//alert($("."+table2+" tr[id='"+editingtrid+"'] td[class='"+editingtdcol+"']").html());
				$("."+table2+" tr[id='"+editingtrid+"'] td[class='"+editingtdcol+"']").html(newval);

				//$("."+table2+" tr[id='"+editingtrid+"'] td[class='"+editingtdcol+"']").html(newval);
				// remove editing flag
				tdediting = 0;
				$("."+table2+" tr[id='"+editingtrid+"'] td[class='"+editingtdcol+"']").effect("highlight",{color: '#acfdaa'},1000);
			break;
		  }
		},
		error: function(){
